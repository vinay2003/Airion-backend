...
const Home: React.FC = () => {
    ...
    useEffect(() => {
        const loadEvents = async () => {
            ...
            try {
                ...
                setError('Failed to load marketplace events');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <main className="min-h-screen bg-white dark:bg-slate-950 transition-colors duration-300">
            ...
            {/* Dashboard Highlights - Simplified */}
            {isAuthenticated && activeCategory === 'all' && (
                <motion.section
                    initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                    className="max-w-7xl mx-auto px-4 md:px-8 -mt-12 mb-16 relative z-40"
                >
                    <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl shadow-red-500/10 border border-red-50 dark:border-slate-800 p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-red-600 rounded-2xl flex items-center justify-center text-white shadow-lg">
                                <LayoutDashboard size={24} />
                            </div>
                            <div>
                                <h2 className="text-xl font-bold text-gray-900 dark:text-white leading-tight">Welcome back, {user?.name}!</h2>
                                <p className="text-xs text-gray-500 font-medium tracking-wide">Continue planning your legendary event.</p>
                            </div>
                        </div>
                        <Link to="/dashboard" className="px-8 py-3 bg-slate-900 text-white rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-red-600 transition-all flex items-center gap-2">
