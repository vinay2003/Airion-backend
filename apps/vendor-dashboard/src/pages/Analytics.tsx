...
const Analytics: React.FC = () => {
 ...
 key={p}
 onClick={() => setPeriod(p)}
 className={`px-5 py-2.5 text-[10px] font-bold tracking-widest rounded-[1.2rem] transition-all ${period === p ? 'bg-indigo-500 text-white shadow-indigo-500/30' : 'text-[var(--ease2event-text-secondary)] hover:text-indigo-500'}`}
 >
 {p}
 </button>
 ))}
 ...
 className="h-12 sm:h-10 px-6 sm:px-5 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-semibold tracking-normal w-full xl:w-auto hover:bg-[var(--ease2event-brand-primary)] hover:text-white transition-all"
 >
 Update Reports
 </Button>
 </div>
