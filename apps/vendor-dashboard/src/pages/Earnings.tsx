...
const Earnings: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10 py-12 border-b border-[var(--airion-border-subtle)] relative overflow-hidden">
        <motion.div variants={itemVariants} className="relative z-10">
          <h1 className="text-5xl font-black text-[var(--airion-text-primary)] tracking-tight leading-none uppercase italic font-display">Financial Intelligence</h1>
          <div className="flex items-center gap-4 mt-6">
            ...
            <p className="text-[var(--airion-text-muted)] font-black text-[12px] uppercase tracking-[0.4em] leading-none opacity-50 italic">Liquidity Matrix • Revenue Protocol</p>
          </div>
        </motion.div>
        
        <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-4 relative z-10">
          <Button className="hidden sm:flex bg-[var(--airion-bg-elevated)] h-12 px-8 rounded-2xl border border-[var(--airion-border-subtle)] font-black text-[11px] uppercase tracking-widest text-[var(--airion-text-muted)] hover:text-[var(--airion-text-primary)] transition-all hover:border-[var(--airion-brand-primary)]/30 active:scale-95">
            <CalendarIcon size={16} className="mr-3" />
            Custom Period
          </Button>
          <Button className="h-12 px-10 rounded-2xl font-black text-[11px] uppercase tracking-widest bg-[var(--airion-brand-primary)] text-white shadow-2xl shadow-[var(--airion-brand-primary)]/20 hover:scale-105 transition-all active:scale-95">
        ...
        {/* Background Decor */}
        <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-[var(--airion-brand-primary)]/[0.03] to-transparent pointer-events-none"></div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        <motion.div 
          ...
          className="card-minimal !bg-[var(--airion-text-primary)] !text-[var(--airion-text-inverted)] p-10 relative overflow-hidden group border-none shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)]"
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex items-center gap-3 mb-6">
                ...
                <p className="text-[var(--airion-text-inverted)]/50 font-black text-[11px] uppercase tracking-[0.4em] italic">Secured Liquidity Node</p>
            </div>
            ...
            <div className="flex items-center justify-between mt-auto pt-8 border-t border-[var(--airion-text-inverted)]/10">
        ...
        <motion.div variants={itemVariants} whileHover={{ y: -8, scale: 1.02 }} className="card-minimal p-10 flex flex-col justify-between hover:border-[var(--airion-brand-primary)]/40 hover:shadow-2xl transition-all duration-500 bg-[var(--airion-bg-surface)]">
          <div>
            <div className="flex justify-between items-start mb-10 relative">
              <div className="p-5 bg-[var(--airion-brand-primary)]/10 text-[var(--airion-brand-primary)] rounded-[24px] shadow-sm border border-[var(--airion-brand-primary)]/10 group-hover:bg-[var(--airion-brand-primary)] group-hover:text-white transition-all duration-500">
                <TrendingUp size={28} />
              </div>
            ...
            <p className="text-[var(--airion-text-muted)] font-black text-[11px] uppercase tracking-[0.4em] mb-4 opacity-50 italic">{activePeriod} Gross Delta</p>
            <h2 className="text-5xl font-black text-[var(--airion-text-primary)] tracking-tighter leading-none italic font-display">₹{displayData.periodRevenue.toLocaleString('en-IN')}</h2>
          </div>
          <div className="flex items-center gap-4 mt-10 pt-8 border-t border-[var(--airion-border-subtle)]">
        ...
        <motion.div variants={itemVariants} whileHover={{ y: -8, scale: 1.02 }} className="card-minimal p-10 flex flex-col justify-between hover:border-amber-500/40 hover:shadow-2xl transition-all duration-500 bg-[var(--airion-bg-surface)]">
          <div className="flex justify-between items-start mb-8">
            <div>
                <p className="text-[var(--airion-text-muted)] font-black text-[11px] uppercase tracking-[0.4em] mb-2 opacity-50 italic">Target Matrix</p>
                <h3 className="text-3xl font-black text-[var(--airion-text-primary)] tracking-tighter italic font-display uppercase leading-none">₹{displayData.payoutTarget.toLocaleString()} GOAL</h3>
            </div>
          ...
          <div className="space-y-6">
              <div className="h-5 w-full bg-[var(--airion-bg-elevated)] rounded-full overflow-hidden p-1.5 border border-[var(--airion-border-subtle)] shadow-inner">
                <motion.div 
                  initial={{ width: 0 }}
              ...
              <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.3em] text-[var(--airion-text-muted)] italic">
                  <span>{displayData.payoutProgress}% SYNC_LEVEL</span>
                  <span className="text-[var(--airion-brand-primary)] tracking-tight">SETTLEMENT: {displayData.payoutDate}</span>
              </div>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
        <motion.div variants={itemVariants} className="xl:col-span-2 card-minimal !p-10 flex flex-col bg-[var(--airion-bg-surface)] shadow-2xl overflow-hidden relative">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-10 mb-16 relative z-10">
                <div>
                    <h3 className="text-3xl font-black text-[var(--airion-text-primary)] tracking-tight italic font-display uppercase leading-none">Revenue Trajectory</h3>
                    <p className="text-[11px] text-[var(--airion-text-muted)] font-black uppercase tracking-[0.4em] mt-4 opacity-50 italic">High-Fidelity Performance Monitoring</p>
                </div>
                <div className="bg-[var(--airion-bg-elevated)] p-2 rounded-3xl border border-[var(--airion-border-subtle)] flex gap-1 shadow-inner relative z-20">
                    {(['Daily', 'Weekly', 'Monthly'] as Period[]).map(period => (
                    <button 
                        key={period} 
                        onClick={() => setActivePeriod(period)}
                        className={`px-10 py-4 text-[11px] font-black uppercase tracking-widest rounded-2xl transition-all duration-700 italic relative ${activePeriod === period ? 'bg-[var(--airion-brand-primary)] text-white shadow-2xl shadow-[var(--airion-brand-primary)]/30 scale-105 z-10' : 'text-[var(--airion-text-muted)] hover:text-[var(--airion-text-primary)]'}`}
                    >
                        {period}
                    </button>
                    ))}
            ...
            <div className="h-[450px] w-full -ml-4 flex-1 relative z-10">
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    <XAxis 
                        ...
                        axisLine={false} 
                        tickLine={false} 
                        tick={{fill: 'var(--airion-text-muted)', fontSize: 10, fontWeight: 900}} 
                        dy={20} 
                    />
                    <YAxis 
                        axisLine={false} 
                    ...
                    <Tooltip 
                        ...
                        labelStyle={{ color: 'var(--airion-text-muted)', fontWeight: 900, fontSize: '11px', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.3em', fontStyle: 'italic' }}
        ...
        <motion.div variants={itemVariants} className="space-y-10">
            <div className="card-minimal p-10 bg-gradient-to-br from-[var(--airion-brand-primary)]/[0.04] to-transparent border-[var(--airion-border-base)] shadow-xl relative overflow-hidden group">
                <div className="absolute -top-10 -right-10 p-12 opacity-[0.02] group-hover:opacity-[0.05] transition-opacity duration-1000">
                    <Activity size={240} />
                </div>
                <h3 className="text-2xl font-black text-[var(--airion-text-primary)] mb-10 tracking-tighter italic font-display uppercase leading-none">Settlement Matrix</h3>
                    ...
                    {[
                        ...
                        <div key={idx} className="flex items-start justify-between group/item">
                            <div className="flex gap-6">
                                <div className={`w-14 h-14 rounded-[20px] flex items-center justify-center shrink-0 bg-[var(--airion-bg-elevated)] border border-[var(--airion-border-subtle)] ${item.color} group-hover/item:scale-110 group-hover/item:rotate-6 transition-all duration-500 shadow-sm`}>
                                    <item.icon size={26}/>
                                </div>
                                <div className="space-y-1.5 pt-1">
                                    <p className="text-[11px] font-black text-[var(--airion-text-primary)] uppercase tracking-tight italic">{item.label}</p>
                                    <p className="text-[10px] font-black text-[var(--airion-text-muted)] uppercase tracking-[0.3em] opacity-50 italic">{item.date}</p>
                                </div>
                            </div>
                            <span className="text-lg font-black text-[var(--airion-text-primary)] italic font-display pt-1">{item.amount}</span>
                        </div>
                    ))}
                </div>
            ...
            <div className="card-minimal p-10 hover:shadow-2xl transition-all duration-700 bg-[var(--airion-bg-surface)] border-[var(--airion-border-base)]">
                <div className="flex items-center gap-5 mb-10">
                    <div className="p-4 bg-emerald-500/10 text-emerald-500 rounded-2xl border border-emerald-500/10 shadow-sm">
                        <BarChart3 size={24} />
                    </div>
                    <h3 className="font-black text-[var(--airion-text-primary)] tracking-tight italic uppercase text-xl font-display leading-none">Net Optimization</h3>
                </div>
                <div className="space-y-8">
                   ...
                   <p className="text-[11px] text-[var(--airion-text-muted)] font-black uppercase italic tracking-tighter leading-relaxed opacity-60">
      ...
      <motion.div variants={itemVariants} className="card-minimal !p-0 overflow-hidden border-[var(--airion-border-base)] shadow-[0_48px_80px_-24px_rgba(0,0,0,0.15)] bg-[var(--airion-bg-surface)] rounded-[40px]">
        <div className="p-10 border-b border-[var(--airion-border-subtle)] bg-[var(--airion-bg-surface)] flex flex-col xl:flex-row items-start xl:items-center justify-between gap-10">
          ...
          <div className="flex gap-5 w-full xl:w-auto">
            <div className="relative flex-1 xl:w-96 group">
                <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-[var(--airion-text-muted)] group-focus-within:text-[var(--airion-brand-primary)] transition-colors" size={20} />
                <input 
                    type="text" 
          ...
          <table className="w-full text-left min-w-[1100px]">
            <thead>
              <tr className="bg-[var(--airion-bg-elevated)]/40 border-b border-[var(--airion-border-subtle)]">
                <th className="px-12 py-8 text-[11px] font-black text-[var(--airion-text-muted)] uppercase tracking-[0.4em] italic">Node ID</th>
                <th className="px-12 py-8 text-[11px] font-black text-[var(--airion-text-muted)] uppercase tracking-[0.4em] italic">Operational Protocol</th>
                <th className="px-12 py-8 text-[11px] font-black text-[var(--airion-text-muted)] uppercase tracking-[0.4em] italic">Capture Source</th>
                <th className="px-12 py-8 text-[11px] font-black text-[var(--airion-text-muted)] uppercase tracking-[0.4em] italic">Timestamp</th>
                <th className="px-12 py-8 text-[11px] font-black text-[var(--airion-text-muted)] uppercase tracking-[0.4em] italic">Capture Method</th>
                <th className="px-12 py-8 text-[11px] font-black text-[var(--airion-text-muted)] uppercase tracking-[0.4em] italic">Gross Capture</th>
                <th className="px-12 py-8 text-[11px] font-black text-[var(--airion-text-muted)] uppercase tracking-[0.4em] italic text-center">Status</th>
              </tr>
            </thead>
              ...
              <AnimatePresence mode="popLayout">
                {displayData.transactions.map((trx: any, tIdx: number) => (
                    <motion.tr 
                        ...
                        className="hover:bg-[var(--airion-brand-primary)]/[0.04] transition-all duration-700 cursor-pointer group"
                    >
                    <td className="px-12 py-10">
                        <span className="font-black text-[var(--airion-brand-primary)] text-[12px] tracking-tighter italic opacity-80 group-hover:opacity-100 transition-opacity">#{trx.id.split('-')[1]}</span>
                    </td>
                    <td className="px-12 py-10 font-black text-sm text-[var(--airion-text-primary)] italic tracking-tight uppercase group-hover:translate-x-3 transition-transform duration-1000">
                        <div className="flex items-center gap-5">
                            <div className="w-2.5 h-2.5 rounded-full bg-[var(--airion-brand-primary)]/20 group-hover:bg-[var(--airion-brand-primary)] group-hover:shadow-[0_0_15px_var(--airion-brand-primary)] transition-all duration-700"></div>
                            {trx.service}
                        </div>
                    </td>
                    <td className="px-12 py-10 text-[11px] font-black text-[var(--airion-text-primary)] uppercase tracking-widest italic opacity-70 group-hover:opacity-100 transition-opacity">{trx.client}</td>
                    <td className="px-12 py-10 text-[var(--airion-text-muted)] text-[10px] font-black uppercase tracking-[0.2em] italic opacity-60">{trx.date}</td>
                    <td className="px-12 py-10">
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] bg-[var(--airion-bg-elevated)] px-4 py-2 rounded-2xl border border-[var(--airion-border-subtle)] text-[var(--airion-text-muted)] group-hover:text-[var(--airion-text-primary)] group-hover:border-[var(--airion-brand-primary)]/20 transition-all italic">{trx.method}</span>
                    </td>
                    <td className="px-12 py-10 font-black text-3xl text-[var(--airion-text-primary)] tracking-tighter italic font-display group-hover:scale-110 group-hover:text-[var(--airion-brand-primary)] transition-all duration-500 origin-left">₹{trx.amount.replace('₹', '')}</td>
                    <td className="px-12 py-10">
        ...
        <div className="p-12 bg-[var(--airion-bg-elevated)]/20 border-t border-[var(--airion-border-subtle)]">
          <button className="flex items-center justify-center gap-5 w-full py-6 rounded-[32px] bg-[var(--airion-bg-surface)] hover:bg-[var(--airion-bg-elevated)] text-[var(--airion-text-primary)] text-[12px] font-black hover:gap-8 transition-all uppercase tracking-[0.5em] border border-[var(--airion-border-base)] shadow-2xl active:scale-[0.99] italic group/btn">
