...
export const fetchEvents = async (filters: Record<string, any> = {}): Promise<Event[]> => {
    ...
    eventCategories.forEach(cat => {
        const count = expandedMockServices.filter(s => s.category === cat).length;
        for (let i = count; i < 10; i++) {
            const baseItem = mockServices.find(s => s.category === cat) || mockServices[0];
            expandedMockServices.push({
                ...baseItem,
                id: `gen-${cat}-${i}`,
    ...
    let mockResults = [...expandedMockServices];
