        ...
        <div className="space-y-16 animate-in fade-in slide-in-from-bottom-6 duration-1000 pb-24">
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-10 border-b border-[var(--airion-border-subtle)] padding-bottom-12">
                <div className="space-y-6">
                    <h1 className="text-4xl font-black text-[var(--airion-text-primary)] tracking-tighter leading-none uppercase italic">Visual Repository</h1>
            ...
            {/* Empty State */}
            {gallery.length === 0 ? (
                <div className="card-minimal !p-32 flex flex-col items-center justify-center text-center space-y-10 bg-[var(--airion-bg-elevated)]/20 border-4 border-dashed border-[var(--airion-border-base)] rounded-[4rem] shadow-inner">
                    ...
                    <div className="max-w-xl space-y-6">
                        <h3 className="text-4xl font-black text-[var(--airion-text-primary)] uppercase tracking-tighter italic leading-none">Repository Null</h3>
                        <p className="text-lg text-[var(--airion-text-muted)] font-bold uppercase tracking-tight leading-loose opacity-70">Synchronize visual telemetry to enhance marketplace visibility index and establish regional presence.</p>
                    </div>
                    <Button
                        onClick={() => fileInputRef.current?.click()}
                        className="btn-secondary !h-16 px-14 text-xs font-black uppercase tracking-[0.4em] border-2 border-[var(--airion-border-base)] rounded-3xl hover:bg-blue-600 hover:text-white transition-all shadow-xl italic"
                    >
                        Force Node Sync
                    </Button>
                </div>
                ...
                <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-10 space-y-10">
                    {gallery.map((item, index) => (
                        <motion.div
                            ...
                            className="relative group cursor-pointer break-inside-avoid rounded-[3rem] overflow-hidden border-2 border-[var(--airion-border-subtle)] shadow-2xl transition-all duration-700 hover:border-blue-500/50 hover:shadow-[0_40px_80px_rgba(0,0,0,0.3)] hover:scale-[1.02] bg-[var(--airion-bg-surface)]"
                        >
                            <img
                                src={item.imageUrl}
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-[var(--airion-bg-surface)] via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex flex-col justify-end p-10">
                                <div className="space-y-6 translate-y-10 group-hover:translate-y-0 transition-transform duration-700">
                                    ...
                                    <div className="flex justify-between items-center">
                                        <span className="text-[var(--airion-text-primary)] text-sm font-black uppercase tracking-[0.2em] truncate pr-6 italic">
            ...
            <AnimatePresence>
                {selectedImage && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[100] flex items-center justify-center p-6 md:p-12 bg-[var(--airion-bg-base)]/95 backdrop-blur-3xl"
                    ...
                    >
                        ...
                        >
                            <button
                                onClick={() => setSelectedImage(null)}
                                className="absolute -top-6 right-0 text-[var(--airion-text-muted)] hover:text-white transition-all flex items-center gap-6 font-black text-xs uppercase tracking-[0.4em] p-6 hover:scale-105"
                            >
                                CLOSE NODE SEQUENCE <X size={44} className="p-2.5 bg-rose-600 text-white rounded-2xl shadow-2xl shadow-rose-600/30" />
                            </button>
                            <div className="w-full h-[80vh] rounded-[4rem] overflow-hidden border-4 border-[var(--airion-border-base)] shadow-[0_50px_100px_rgba(0,0,0,0.6)] bg-[var(--airion-bg-surface)] group relative">
                                <img
                            ...
                            {selectedImage.title && (
                                <motion.div
                                    ...
                                    className="mt-12 flex items-center gap-6 bg-[var(--airion-bg-surface)] px-14 py-6 rounded-[2.5rem] border-2 border-[var(--airion-border-base)] shadow-[0_30px_60px_rgba(0,0,0,0.4)]"
                                >
                                    <div className="w-4 h-4 bg-blue-500 rounded-full animate-ping" />
                                    <p className="text-[var(--airion-text-primary)] font-black text-xl uppercase tracking-[0.2em] italic">{selectedImage.title}</p>
                                    <div className="h-6 w-0.5 bg-[var(--airion-border-subtle)] mx-2" />
                                    <span className="text-xs font-black text-[var(--airion-text-muted)] uppercase tracking-widest">{new Date(selectedImage.createdAt).toLocaleDateString()}</span>
                                </motion.div>
                            )}
                        </motion.div>
                    </motion.div>
