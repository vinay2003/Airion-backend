    ...
    {isLoading ? (
      ...
      <>
        <div className="space-y-4 z-10">
          ...
          <h3 className="text-3xl md:text-4xl font-bold text-[var(--ease2event-text-primary)] tracking-tight leading-none">
            {currency}{value}
          </h3>
        </div>
        ...
        <div className="flex items-center justify-between z-10">
          {trend ? (
            <div className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg ${direction === 'up' ? 'text-emerald-500 bg-emerald-500/10 border border-emerald-500/20' : 'text-rose-500 bg-rose-500/10 border border-rose-500/20'}`}>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 space-y-12 pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-[var(--ease2event-border-subtle)] pb-10">
        <div className="space-y-4">
          ...
          <p className="text-base font-semibold text-[var(--ease2event-text-secondary)]">Overview of your business performance and activity.</p>
        </div>
          ...
          <div className="card-minimal !p-10 overflow-hidden group">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
              <div className="space-y-1">
                <h2 className="text-xl font-bold text-[var(--ease2event-text-primary)] tracking-tight">Revenue Overview</h2>
            ...
            <div className="bg-blue-600 rounded-[2.5rem] p-10 text-white space-y-10 shadow-[0_20px_50px_rgba(37,99,235,0.3)] flex flex-col justify-between relative overflow-hidden group transition-all duration-300 hover:scale-[1.02]">
              ...
              <div className="flex justify-between items-start z-10">
                ...
                <div className="text-right">
                  <p className="text-sm font-semibold opacity-70">Next Event</p>
                  <p className="text-2xl font-bold mt-1 leading-none">JAN 20, 2026</p>
                </div>
              </div>
        ...
        <div className="space-y-10">
              ...
              {enquiries.map((e, i) => (
                <div key={i} className="flex items-center gap-5 group cursor-pointer p-2 -m-2 rounded-2xl hover:bg-[var(--ease2event-bg-elevated)] transition-all">
                  ...
                  <div className="flex-1 min-w-0 space-y-2">
                    <h4 className="text-xl font-bold text-[var(--ease2event-text-primary)] tracking-tight leading-none">{e.name}</h4>
                    <p className="text-sm text-[var(--ease2event-text-secondary)] font-semibold">{e.event} • <span className="text-blue-600 font-bold">{e.time}</span></p>
                  </div>
          ...
          <div className="card-minimal !p-10 space-y-10 relative overflow-hidden group">
            ...
            <div className="space-y-4 z-10 relative">
              <h2 className="text-2xl font-bold text-[var(--ease2event-text-primary)] leading-none tracking-tight">Store Visibility</h2>
              <p className="text-sm font-semibold text-[var(--ease2event-text-secondary)]">Your performance ranking on Airion</p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[var(--ease2event-bg-elevated)] border border-[var(--ease2event-border-subtle)] p-8 rounded-3xl space-y-6 group hover:border-blue-500/30 transition-all duration-500">
              <div className="flex justify-between items-start">
                <p className="text-lg font-bold text-[var(--ease2event-text-primary)] tracking-tight">Setup Decoration Flow</p>
                <span className="text-sm font-bold text-[var(--ease2event-text-secondary)]">JAN 18</span>
              </div>
              <div className="flex items-center gap-5">
                ...
                <p className="text-md text-[var(--ease2event-text-secondary)] font-bold">Awaiting Vendor Approval</p>
              </div>
            </div>
          </div>
        </div>
