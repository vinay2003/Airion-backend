...
const VendorProfileEditor = () => {
    ...
    const fetchSubcategories = async (categoryId: string) => {
        try {
            const response: any = await api.get(`/categories/${categoryId}/subcategories`);
            setSubcategories(Array.isArray(response) ? response : (response?.data || []));
        } catch (e) { console.error(e) }
    };
    ...
    const fetchVendorProfile = async () => {
        try {
            ...
            const profile = response?.data !== undefined ? response.data : response;
            if (profile && !profile.message && Object.keys(profile).length > 0) {
    ...
    const onSubmit = async (data: any) => {
        ...
        try {
            ...
            try {
                const current: any = await api.get('/vendors/me');
                const profile = current?.data !== undefined ? current.data : current;
                if (profile && Object.keys(profile).length > 0 && !profile.message) {
                    profileExists = true;
                }
            } catch (err) {
                profileExists = false;
