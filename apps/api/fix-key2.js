...
// Remove ALL non-base64 chars (like \n, \Y, spaces, etc.)
let cleanBody = body.replace(/[^A-Za-z0-9+/=]/g, '');

const matchedContent = cleanBody.match(/.{1,64}/g);
const fixedKey = `-----BEGIN PRIVATE KEY-----\n${matchedContent.join('\n')}\n-----END PRIVATE KEY-----\n`;

try {
  crypto.createPrivateKey(fixedKey);
  console.log("SUCCESS!");
  fs.writeFileSync('fixed-key.txt', `FIREBASE_PRIVATE_KEY="${fixedKey.replace(/\n/g, '\\n')}"`);
} catch(err) {
