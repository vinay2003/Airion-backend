...
const VendorSignupWizard: React.FC = () => {
    ...
    useEffect(() => {
        const checkExistingProfile = async () => {
            if (!user) return;
            try {
                const response = await api.get('/vendors/me') as any;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const submissionData = {
                ...
                socialLinks: {
                    website: formData.website?.trim() ? (formData.website.startsWith('http') ? formData.website : `https://${formData.website}`) : undefined
                }
            };
            ...
            console.error('[VendorSignup] Full Error Response Details:', err);
            const detailMessage = err.error || err.response?.data?.message || err.message;
            toast.error(detailMessage || 'Failed to save profile. Please check all fields.');
        } finally {
            setSubmitting(false);
        }
    };
